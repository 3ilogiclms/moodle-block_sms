...
// Role names (value -> field in {role} table):
// 1 -> shortname
// 2 -> name
// 3 -> archetype
...
class sms_send extends moodleform {
    ...
    public function definition() {
        ...
        $select = $mform->addElement('select', 'g_selection', get_string('group_selection', 'block_sms'), $attributes, null, array(null));
        ...
        $attributes = array('0' => 'Select Group');
        $select = $mform->addElement('select', 'g_id', get_string('group', 'block_sms'), $attributes, null, array(null));

        switch ($CFG->block_sms_rolenames) {
            case '0':
                $rolenames = "shortname";
                break;
            case '1':
...
//Originally: $attributes1=array('manager', 'teacher', 'student');
//Create an array with all the roles allowed (based on $rolenames)
            $attributes1 = array();
            $attributesFromDB = $DB->get_records_sql('SELECT id, ' . $rolenames . ' FROM {role}', null, $limitfrom = 0, $limitnum = 0);
            foreach ($attributesFromDB as $attribute) {
//$attribute is a standard object, hence access is possible with operator '->'
                if (in_array($attribute->id, explode(',', $CFG->block_sms_roleselection)))
                    $attributes1[] = $attribute->$rolenames;
            }
        }

        $attributes2 = $DB->get_records_sql_menu('SELECT id , ' . $rolenames . ' FROM {role}', null, $limitfrom = 0, $limitnum = 0);
    ...
    public function display_report($c_id = 1, $r_id = 3, $g_id = 0, $isgroup = 1) {
        global $DB, $OUTPUT, $CFG, $USER;
        $table = new html_table();
...
INNER JOIN {role} r ON ra.roleid = r.id
INNER JOIN {user} usr ON ra.userid = usr.id
INNER JOIN {groups_members} grp ON usr.id = grp.userid
WHERE r.id = $r_id AND grp.groupid = $g_id";
        }
